    ...
    <el-form style="margin-left: -25px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <div style="border: 0px solid red;width: 400px;height: 60px;position: relative;top: -64px;left: 450px">
        ...
        <div style="position: relative ;left: -424px">
          ...
          <el-dialog :visible.sync="visibleDialog" v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" title="添加产品">
            <el-row :gutter="15">
              <el-form ref="elForm" :model="formData" :rules="rules" size="medium"  label-width="100px">
                <el-col :span="14">
                  <el-form-item label="产品名称" prop="goodName">
                    <el-input v-model="formData.goodName" placeholder="请输入产品名称" clearable
        ...
        <template slot-scope="scope">
          ...
          <el-drawer append-to-body
            ...
            <el-descriptions title="" direction="vertical" :column="2" border>
              ...
              <el-descriptions-item label="货物类型">产品</el-descriptions-item>
...
export default {
  ...
  methods:{
    ...
    handleConfirm() {
      this.$refs['elForm'].validate(valid => {
        ...
        if (this.formData.goodId == ''){
          axios.post('http://localhost:8181/good',data)
              .then(resp=>{
                let data = resp.data;
                let code = data.code;
                if (code == 200){
    ...
    initTable() {
      axios.get("http://localhost:8181/good/selectAll/"+this.currentPage)
          .then(resp=>{
            let data = resp.data;
    ...
    submitForm(pageNum){
      ...
      axios.get('http://localhost:8181/good/query',{params:this.ruleForm})
          .then(resp=>{
            let data = resp.data;
            let code = data.code;
            if (code == 200){
